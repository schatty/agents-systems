...
def learner_worker(config, training_on, policy, target_policy_net, learner_w_queue, replay_priority_queue,
    ...
    learner = LearnerTD5(config, policy, target_policy_net, learner_w_queue, log_dir=experiment_dir)
...
class Engine(object):
    ...
    def train(self):
        ...
        target_policy_net = PolicyNetwork(config['state_dim'], config['action_dim'], config["action_high"],
                                          config['dense_size'], device=config['device'])
        policy_net = copy.deepcopy(target_policy_net)
        target_policy_net.share_memory()
        if config['agent_device'] == 'cpu':
            agent_policy = PolicyNetwork(config['state_dim'], config['action_dim'], config["action_high"],
