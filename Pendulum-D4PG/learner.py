"""
Learner trains the D4PG network on experienes sampled (by priority) from the PER buffer
"""
...
class Learner:
    ...
    def build_network(self):
        ...
        self.weights_ph = tf.placeholder(tf.float32, (train_params.BATCH_SIZE))
        ...
        if train_params.USE_BATCH_NORM:
            ...
            raise Exception("Critic without BN")
        ...
        # Create saver for saving model ckpt (only save learner network vars)
    ...
    def initialize_vars(self):
        if train_params.CKPT_FILE is not None:
            ...
            # Peform hard copy (tau=1.0) of initial params to target networks
        ...
        for train_step in range(self.start_step + 1, train_params.NUM_STEPS_TRAIN+1):
            ...
            if train_step % train_params.REPLAY_MEM_REMOVE_STEP == 0:
                if len(self.PER_memory) > train_params.REPLAY_MEM_SIZE:
                    # Prevent agent from adding new xperiences to replay memory while learner removes samples
